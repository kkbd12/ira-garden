            ...
            isNoticeFormVisible ? (
                ...
                <Card className="p-6 rounded-t-none">
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <button onClick={handleAddNewNotice} className="flex items-center gap-2 px-4 py-2 bg-teal-600 text-white rounded-md font-semibold hover:bg-teal-700 transition-colors duration-300">
                            <AddIcon />
                    ...
                    <div className="space-y-4">
                        {displayedNotices.length > 0 ? displayedNotices.map(notice => {
                            ...
                            return (
                                <div key={notice.id} className={`p-4 border rounded-lg flex justify-between items-center ${isExpired ? 'bg-red-50 border-red-200' : 'bg-gray-50'}`}>
                                    <div>
                                    ...
                                    <div className="flex gap-2">
                                        <button onClick={() => handleEditNotice(notice)} className="p-2 text-blue-600 hover:text-blue-800" aria-label="সম্পাদনা করুন"><EditIcon /></button>
                                        <button onClick={() => deleteNotice(notice.id)} className="p-2 text-red-600 hover:text-red-800" aria-label="মুছে ফেলুন"><DeleteIcon /></button>
                                    </div>
                                </div>
                            );
                        }) : (
                        ...
                        {photos.map(photo => (
                            <div key={photo.id} className="relative group border rounded-lg overflow-hidden shadow">
                                ...
                                <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-50 transition-all flex justify-center items-center">
                                    <button onClick={() => deletePhoto(photo.id)} className="p-2 bg-red-600 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity" aria-label="মুছে ফেলুন">
                                        <DeleteIcon />
                                    </button>
                                </div>
                            </div>
